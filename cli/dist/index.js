...
var COOKIE_FILE = "./cookie/cookies.json";
...
// const/Urls.ts
var BASE_URL = "http://qlikserver.dev";

// routes/users.ts
var usersCommandConfig = (program3) => {
  program3.command("users").description("Prints a hello message").action(() => {
    for (let i = 0; i < baseUser.length; i++) {
      console.log(`User ${i + 1}: ${baseUser[i].name}, Password: ${baseUser[i].password}`);
    }
  });
  program3.command("login").description("Login to the system").option("-u, --username <username>", "Your username").option("-p, --password <password>", "Your password").action((options) => __async(void 0, null, function* () {
    var _a, _b;
    ...
    try {
      const response = yield axios_default.post(`${BASE_URL}/api/users/signin`, {
        email: options.username,
        password: options.password
      }, {
        withCredentials: true
      });
      const cookie = (_a = response.headers["set-cookie"]) == null ? void 0 : _a[0];
      if (cookie) {
        saveCookies(cookie);
      ...
      console.error("Login failed:", ((_b = error.response) == null ? void 0 : _b.data) || error.message);
    }
  }));
  program3.command("cookie").description("Get the saved cookie").action(() => {
  ...
  program3.command("howami").description("Check the user status").action(() => __async(void 0, null, function* () {
    const cookie = loadCookies();
    const howAmI = yield axios_default.get(`${BASE_URL}/api/users/currentuser`, {
      headers: {
        "Cookie": cookie
      },
      withCredentials: true
...
// routes/messages.ts
var messagesConfig = (program3) => {
  program3.command("publishmessage").description("publish new nessage").option("-c, --content <content>", "Your  msg content").action((options) => __async(void 0, null, function* () {
    const cookie = loadCookies();
    if (!cookie || !options.content) {
      console.error("Error: No cookie found. Please login first.");
      process.exit(1);
    }
    const result = yield axios_default.post(`${BASE_URL}/api/messages/postMessage`, { messageContent: options.content }, {
      headers: {
        "Cookie": cookie
      },
      withCredentials: true
  ...
  program3.command("getmessages").description("Get all my messages").action(() => __async(void 0, null, function* () {
    const cookie = loadCookies();
    if (!cookie) {
      console.error("Error: No cookie found. Please login first.");
      process.exit(1);
    }
    const result = yield axios_default.get(`${BASE_URL}/api/messages/getMyMessage`, {
      headers: {
        "Cookie": cookie
      },
      withCredentials: true
  ...
  program3.command("modifymessage").description("Modify my message").option("-m, --messageId <messageId>", "Your message id").option("-c, --content <content>", "Your new msg content").action((options) => __async(void 0, null, function* () {
    const cookie = loadCookies();
    if (!cookie || !options.messageId || !options.content) {
      console.error("Error: No cookie found. Please login first.");
      process.exit(1);
    }
    const result = yield axios_default.patch(`${BASE_URL}/api/messages/updateMessage`, { id: options.messageId, content: options.content }, {
      headers: {
        "Cookie": cookie
      },
      withCredentials: true
  ...
  program3.command("deletemessage").description("Delete my message").option("-id, --id <id>", "Your message id").action((options) => __async(void 0, null, function* () {
    const cookie = loadCookies();
    if (!cookie || !options.id) {
      console.error("no cookie found. Please login first or messageId is required");
      process.exit(1);
    }
    const result = yield axios_default.delete(`${BASE_URL}/api/messages/deleteMessage`, { data: { id: options.id }, headers: { "Cookie": cookie }, withCredentials: true });
    console.log("result", result.data.deletedCount > 0 ? "message deleted" : "fail to delete message");
  }));
};

// routes/dev.ts
var devConfig = (program3) => {
  program3.command("dbrest").description("rest db with users and messages").option("-m, --messages", "rest db with messages", false).action((options) => __async(void 0, null, function* () {
    try {
      if (options.messages) {
        const result = yield axios_default.post(`${BASE_URL}/api/dev/restDBWithUsersAndMessages`);
        console.log("result", result.data);
      } else {
        const result = yield axios_default.post(`${BASE_URL}/api/dev/restDBWithUsers`);
        console.log("result", result.data);
      }
    } catch (e) {
      {
...
usersCommandConfig(program2);
messagesConfig(program2);
devConfig(program2);
function main() {
  return __async(this, null, function* () {
    console.log("Starting CLI...");
    yield program2.parseAsync(process.argv);
  });
}
main();
/*!
